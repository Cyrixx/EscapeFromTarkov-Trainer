...
Pass{
    ...
    v2f vert(appdata v) {
        ...
        // Check if the vertex is in line of sight
        bool inLineOfSight = dot(normalize(UnityWorldSpaceViewDir(v.vertex)), normalize(v.normal.xyz)) > 0;

        if (inLineOfSight) {
            v.vertex.xyz += scaleDir * _FirstOutlineWidth;
            // Set the color to green if in line of sight
            _FirstOutlineColor = (0, 1, 0, 1);
        }
 else {
